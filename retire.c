...
double adjustedRate = (1 + averageReturn) / (1 + averageInflation) - 1; // inflation adjusted rate of return formula
    ...
    double monthlyRate = (adjustedRate / 12.0);

    double balance = (startingBalance * 100 ) / 100; // multiply by 10 and divude by 10
    double totalInterest = 0.0;
    int months =  retirement * 12;
    ...
    for(int i=1; i<=months; i++) {
      ...
      double balanceBeforeCont = balance + monthlyCont;
      double interest = round(balance * monthlyRate * 100 ) / 100;

      balance = balanceBeforeCont + interest;
      totalInterest += interest;
